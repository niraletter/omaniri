...
rev 3 names the feature **dots** (`omaniri dots ...`) and adds the
multi-machine sync design.

## Problem

Omaniri declares `~/.config` "your files" but does little to preserve them:

- `omaniri-refresh-config` litters `*.bak.<timestamp>` files next to originals.
- `omaniri-reinstall-configs` clobbers everything back to `/etc/skel` defaults.
...
- **`~/.config/omaniri` only**: too narrow; misses hypr, terminals, `.bashrc`.
...
- **Raw git passthrough / lazygit over `$HOME`** (rejected in review):
  ...
  raw invocation themselves; Omaniri will not bless it.
...
```bash
git init --bare ~/.local/share/omaniri/dots.git   # mode 0700
```
...
- Repo-local config only: synthetic identity (`Omaniri <omaniri@localhost>`),
...
- Never export `GIT_DIR`/`GIT_WORK_TREE` (would leak into `omaniri-plugin-*`,
  `omaniri-theme-update`, `omaniri-update-dev`, user hooks).
...
- **Best-effort everywhere**: a failed snapshot warns and continues. History
  ...
  `omaniri-update`/`omaniri-migrate` (`set -e` there must not see git errors).

### Audited manifest, not a derived whitelist

Tracking is `git add -f --pathspec-from-file=<manifest>` only. The manifest is
a hand-audited file shipped with Omaniri — explicitly **not** derived from
`$OMANIRI_PATH/config` (which ships Chromium Preferences, fcitx5,
`opencode/opencode.json` where users put API keys, etc.):

- Include: `.config/hypr/*.{lua,conf}`, `.config/omaniri/shell.json`,
  `.config/omaniri/extensions/**`, `.config/omaniri/hooks/**`,
  terminal configs (alacritty/foot/ghostty/kitty), `.config/btop/btop.conf`,
  `.config/starship.toml`, `.bashrc`, `.XCompose`.
- Exclude (deliberately): `.config/omaniri/plugins/**` and
  `.config/omaniri/themes/**` (nested git clones managed by
  `omaniri-plugin-*` / `omaniri-theme-update`), backgrounds (multi-MB
...
- At seed time and before each snapshot: if manifest paths are symlinks or a
  ...
  detected, mark the repo dormant and say so in `omaniri dots status`.
...
internal `omaniri-dots-snapshot "<label>"`:

1. **`omaniri-provision-user`**: seed repo + initial commit, after `git.sh`
   and `.XCompose` are in place. Idempotent; `--force` must not re-init.
2. **`omaniri-migrate`**: before/after pair labeled with version. Covers both
   `omaniri-update` *and* the login-time migration path, and separates
...
3. **Batch refresh commands** (`omaniri-refresh-hyprland` etc.): one pair per
   command, not one commit per `omaniri-refresh-config` call (hyprland calls
   it seven times).
4. **`omaniri-reinstall-configs`**: a "before" snapshot — honestly labeled.
...
5. **Timer**: an hourly-ish systemd user timer committing only if dirty, so
   ...
   fails for any edit not followed by an Omaniri operation.
...
- **`omaniri dots push`** — *squash-publishes* the current manifest state
...
- **`omaniri dots pull [url]`** — fetches `sync`, takes a local pre-pull
...
one `omaniri dots restore` away.

Remote setup UX:

- `omaniri dots push` with no remote runs setup inline: paste a git URL, or
...
- Dormant mode (dotfile-manager users) disables sync too — they have their
  own.
- v2 candidate: `omaniri-provision-first-run` asks "Got a dots repo?" so a
...
invoked as `omaniri dots <verb>`:

- `omaniri dots snapshot [label]` — take a snapshot now
- `omaniri dots log` — history with labels
- `omaniri dots diff [ref]` — default range: the last before/after pair
- `omaniri dots restore <file> [--at <ref>]` — accepts `~/.config/...`,
...
- `omaniri dots push` / `omaniri dots pull [url]` — sync (above)
- `omaniri dots status` — repo state, dormant/active, remote, last
  snapshot/publish
...
- New users: seeded by `omaniri-provision-user`.
...
- Docs/tests alignment: manual `31-dotfiles.md` gets the built-in story
  ...
  per the documentation layout; `default/agents/skills/omaniri/SKILL.md`
...
1. Manifest location: `default/omaniri/dots-manifest` vs alongside the
   helper; and whether users may extend it via
   `~/.config/omaniri/dots-manifest.d/`.
2. Whether `.config/omaniri/hooks/**` belongs in the manifest at all (small
   scripts, but the likeliest place for a pasted token).
3. Harden `omaniri-refresh-config` against `..` path escape (documented today
