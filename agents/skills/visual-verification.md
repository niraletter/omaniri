...
Read this before finishing any change with a visual effect: Omaniri shell
...
```bash
omaniri capture screenshot fullscreen save
```

The command prints the saved path and writes to the configured Pictures
directory. Use `omaniri screenshot` for the interactive smart-region flow.
...
```bash
omaniri screenrecord --fullscreen
# Exercise the changed behavior.
omaniri screenrecord --stop-recording
```
