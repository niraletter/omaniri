...
Run `omaniri-restart-shell` after making changes to QML files.
...
- First-party plugins live directly under `shell/plugins/` or one category
  ...
  at `~/.config/omaniri/plugins/<id>/` with a `manifest.json` at the root.
- Every plugin manifest declares `schemaVersion`, `id`, `name`, `version`,
  `kinds`, and `entryPoints`. See
  [`docs/omaniri-shell.md`](../../docs/omaniri-shell.md) and
...
- Entry-point QML files are `Item`s (not `ShellRoot`), and accept the
  shell-injected properties `omaniriPath`, `shell`, `manifest`, and
...
- `bin/omaniri-shell` is the canonical IPC entry point. It forwards to
...
- The `shell` IPC target exposes lifecycle and configuration methods including
  ...
  `image-selector`, which drives the `omaniri.image-picker` panel.
- Individual plugins register their own IPC targets, named for the plugin rather
  ...
  bar widgets register one target each — `omaniri.indicators`,
  `omaniri.system-update`, `omaniri.clock`. There is no `bar` target.
