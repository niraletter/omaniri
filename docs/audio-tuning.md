...
`on` renders the graph into `~/.config/pipewire/omaniri-speaker-tuning.conf.d/`
(plus the shared host config `omaniri-speaker-tuning.conf`, from
`default/audio/filter-chain-host.conf`) and runs it as its own PipeWire client via
`omaniri-speaker-tuning.service`, rather than
...
```bash
omaniri audio tuning on        # install the matching tuning
omaniri audio tuning off       # remove it, back to raw speakers
omaniri audio tuning status    # installed? in use? what matches?
```
...
node is named `omaniri_speaker_tuning` (the install checks for that exact name,
...
```bash
omaniri pkg add omarchy-audio-tuner
```
...
- **Volume lives downstream of the tuning.** A tuning is a virtual sink that
  ...
  `omaniri-audio-output-sink` is the single definition of "which sink does this
...
- **The fronted sink is hidden.** The tuning and the physical speakers both exist
  ...
  `omaniri-audio-sink-availability` reports it unavailable and
  `omaniri-audio-output-switch` skips it, leaving one speaker entry in the panel.
...
- **The tuning's own output must not be moved.** A filter-chain's output is a
  ...
  `omaniri-audio-output-set-default` moves only streams that carry an
  `application.name`.
