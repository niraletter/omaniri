...
The menu is the `omaniri.menu` plugin of the Quickshell desktop, with its
content defined as data in `default/omaniri/omaniri-menu.jsonc` (read at
runtime from `$OMANIRI_PATH`) and overlaid by the user's
`~/.config/omaniri/extensions/omaniri-menu.jsonc`. The shell parses both files
...
| `iconFont` | Font family for the glyph when it differs from the menu font — how the private `omaniri` font's brand glyphs render |
...
| `aliases` | Alternate `omaniri menu summon <name>` routes; also searchable |
...
The sample extension at `config/omaniri/extensions/omaniri-menu.jsonc`
...
- Package and command presence (`omaniri-pkg-present` and friends) are
...
- Commands that several rows read a value from — every Defaults > Browser row
  compares against `$(omaniri-default-browser)` — run once, with the captured
  answer substituted into each expression. The reader list is
  `GUARD_READERS` in `MenuModel.js`; a new `$(omaniri-...)` reader used by
...
- `disabled` keeps the row listed but dims it, marks it ✓, and makes it
  ...
  list stays a catalog of what Omaniri can install. Since a dimmed row means
...
`bin/omaniri-menu` is a thin wrapper over the standard plugin IPC surface:

```bash
omaniri menu                    # toggle the root menu
omaniri menu toggle system      # open at a route, or close if already open
omaniri menu summon style.theme # always open (no close-if-visible)
omaniri menu close
omaniri menu refresh            # re-parse the JSONC files
omaniri menu ping
```
...
The same plugin doubles as the system's dmenu. `omaniri-menu-select` and
`omaniri-menu-input` summon it with a `mode: select` or `mode: input`
...
how the pickers behind menu actions (`omaniri-menu-plugin`,
`omaniri-menu-timezone`, ...) present lists without owning any UI.
