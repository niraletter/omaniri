...
that hosts the Omaniri desktop. The bar, panels, overlays, menus, and
services all run inside as plugins. Hyprland autostart launches the
shell via `omaniri-launch-shell`; restart it with `omaniri-restart-shell`.
IPC is the canonical way for CLIs to talk to a running shell —
`omaniri-shell` forwards a call and fails when the shell is not running
(`-q` makes it quiet best-effort; `OMANIRI_SHELL_IPC_TIMEOUT` bounds the
wait).
...
| `bar`        | Full bar option that can replace `omaniri.bar` |
...
Only one full bar option is active at a time. The built-in `omaniri.bar` is
...
`omaniriPath`, `shell`, `manifest`, and the registries (`pluginRegistry` /
...
clones it straight into `~/.config/omaniri/plugins/<id>/`; updating is a
fast-forward pull:

```bash
omaniri plugin add https://github.com/acme/omaniri-weather.git
omaniri plugin update                # fetches, shows a diff, fast-forwards
...
Cloning `omaniri.clock`, for example, creates and switches to
`~/.config/omaniri/plugins/<username>.clock/` (e.g. `nira.clock`), names it
...
Plugins run as **unsandboxed code** inside `omaniri-shell`. Adding warns you
before cloning, plugins land disabled so you can review the code before
`omaniri plugin enable`, and updates show a diff before touching anything.
...
`~/.config/omaniri/plugins/<id>/`, run `omaniri-shell shell rescanPlugins`, then
`omaniri plugin enable <id>`. A bar widget starts in its declared default
section; enabling a full bar replaces the one in use. `omaniri bar` drives the
bar from the CLI — `use | reset | defaults | position | transparent | put |
move | set`, with placement flags such as `--section` and `--index`.
The lower-level IPC methods remain available through `omaniri-shell shell ...`.
...
`media`, `notifications`, and per-widget targets such as `omaniri.clock`
or `omaniri.power`. There is no `bar` target.
...
| `putBarWidget <id> <placementJson>`   | place a widget only where absent (`omaniri bar put`) |
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left":   [ { "id": "omaniri.menu" } ],
      "center": [ { "id": "omaniri.clock", "format": "HH:mm" } ],
      "right":  [ { "id": "omaniri.audio" } ]
    }
  },
  "plugins": [
...
1. The active bar option is `bar.id`. Omit it or set it to `omaniri.bar` for
...
3. Settings are inline on the entry. No `config:` sub-object, no
   merge layers.
4. Built-in bar widget ids are namespaced (`omaniri.clock`, `omaniri.audio`, …).
...
`config/omaniri/shell.json` describes the fresh-install state. When no
...
machine-level `~/.config/omaniri/shell.toml` is watched live by the
shell and its keys win over the active theme's `shell.toml`, so
overrides like `omaniri display text size` survive theme switches.
...
`omaniri-theme-set-templates` (see [`theming.md`](theming.md)).
...
themes don't set it, the user does via `omaniri font set <name>`.
...
```json
{ "id": "vpn", "type": "command", "exec": "~/.config/omaniri/bar/scripts/vpn-status",
...
Then `~/.config/omaniri/bar/modules/gpu.qml` (or set `source` to point
