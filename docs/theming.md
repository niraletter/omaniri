...
Omaniri themes live under `themes/<name>/` in the source tree (installed at
`/usr/share/omaniri/themes/<name>/`), with optional user themes under
`~/.config/omaniri/themes/<name>/`. A theme normally starts with a
`colors.toml`; Omaniri generates the active theme files from
`default/themed/*.tpl` when `omaniri-theme-set <name>` runs.

Beyond `colors.toml` and hand-written config overrides, a theme can ship
`backgrounds/` (users overlay their own via
`~/.config/omaniri/backgrounds/<name>/`; the active image is the
`~/.local/state/omaniri/current/background` symlink), `preview.png` and
...
`omaniri-theme-set <name>` builds a clean staging directory at
`~/.local/state/omaniri/current/next-theme`:

1. Copy the first-party theme from `themes/<name>/`.
2. Overlay any user theme files from `~/.config/omaniri/themes/<name>/`.
3. If needed, generate `colors.toml` from `alacritty.toml`.
4. Run `omaniri-theme-set-templates` to render templates into the staging
   theme.
5. Move the staging theme into `~/.local/state/omaniri/current/theme`, write
   `~/.local/state/omaniri/current/theme.name`, and notify the running shell.
...
User templates in `~/.config/omaniri/themed/*.tpl` are processed before the
built-in templates. If a user template has the same output filename as a
built-in template, the built-in output is skipped.

After activation, `omaniri-theme-set` fires the `theme-set` hook
(`~/.config/omaniri/hooks/theme-set*`, theme name in `$1`) and dispatches a
parallel retint of running apps — terminals, Hyprland, btop, browser, editors,
and the rest of the `post_theme_commands` list in `bin/omaniri-theme-set`.
...
in that ramp; Omaniri derives `selection_background = selection` and
`selection_foreground = bright_foreground`. Use
`omaniri dev theme-preview [theme]` to inspect that ramp, including
`dark_background`, `darker_background`, and a selected-text sample.

## Template placeholders

Templates are plain files ending in `.tpl`. `omaniri-theme-set-templates`
...
- Add a user-wide template under `~/.config/omaniri/themed/<file>.tpl` when a
