--
-- Dynamic omaniri Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the omaniri-shipped Plymouth via
-- omaniri-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs omaniri-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "omaniriunlocks"
NamePretty = "omaniri Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/omaniri/themes"
  local OMANIRI_PATH = os.getenv("OMANIRI_PATH") or ""
  local default_themes_dir = OMANIRI_PATH .. "/themes"
  local default_preview = OMANIRI_PATH .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "omaniri-launch-floating-terminal-with-presentation "
                .. shell_escape("omaniri-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "omaniri-launch-floating-terminal-with-presentation "
        .. shell_escape("omaniri-plymouth-reset"),
    },
  }
