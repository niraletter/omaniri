...
local function command_from(value, description)
  ...
  if value.omaniri then
    return "omaniri-launch-" .. value.omaniri
  ...
  elseif value.tui then
    if value.focus then
      return "omaniri-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "omaniri-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.preinstalled_bindings_enabled()
  if _G.omaniri_preinstalled_bindings ~= nil then
    return _G.omaniri_preinstalled_bindings == true
  end

  return not file_exists((os.getenv("HOME") or "") .. "/.local/state/omaniri/preinstalls-removed")
end
...
function o.launch_webapp(url)
  return "omaniri-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "omaniri-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "omaniri-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "omaniri-toggle-" .. toggle, options)
end
