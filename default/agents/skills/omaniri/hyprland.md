...
Omaniri configures Hyprland in Lua. User files are loaded after Omaniri's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads Omaniri defaults, then user files)
...
- Use `omaniri refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `omaniri restart hyprsunset`; reset with `omaniri refresh hyprsunset`
...
o.bind("SUPER + B", "Browser", { launch = "chromium" })  -- launch spawns the app directly
```

View current bindings: `omaniri menu keybindings --print`

**IMPORTANT: When re-binding an existing key:**

1. First check existing bindings: `omaniri menu keybindings --print`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer Omaniri's `o.window(match, rules)` helper — see examples in `$OMANIRI_PATH/default/hypr/windows.lua`.
