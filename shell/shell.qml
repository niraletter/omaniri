...
ShellRoot {
  ...
  // The omaniri-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. OMANIRI_PATH is provided by the Omaniri
  // session wrapper (default/wayland-sessions/omaniri-session) via systemd
  // import-environment and is the single source of truth for this checkout.
  property string omaniriPath: Quickshell.env("OMANIRI_PATH")
  readonly property string shellPath: omaniriPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: omaniriPath + "/config/omaniri/shell.json"
  readonly property string userConfigPath: home + "/.config/omaniri/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "omaniri.clock", format: "dddd HH:mm" }],
  ...
  Component.onCompleted: {
    console.log("omaniri-shell paths",
      "omaniriPath=" + shell.omaniriPath,
  ...
  readonly property string defaultBarId: "omaniri.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("omaniriPath" in target) target.omaniriPath = shell.omaniriPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("omaniriPath" in inst) inst.omaniriPath = shell.omaniriPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. omaniri.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("omaniriPath" in item) item.omaniriPath = shell.omaniriPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["omaniri.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("omaniri.image-picker", payload) ? "ok" : "unknown"
    }
    ...
    function cancel(doneFile: string): string {
      ...
      if (picker && typeof picker.closeSelector === "function") {
        ...
        shell.hide("omaniri.image-picker")
      }
      return "ok"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].omaniri
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `omaniri plugin enable/disable` toggles: for a widget that is
