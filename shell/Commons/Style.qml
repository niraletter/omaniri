...
//
// CHANGED FOR NIRI: `cornerRadius`/`gapsOut` used to mirror Hyprland's live
// `decoration:rounding` / `general:gaps_out` via `hyprctl -j getoption`
// polling (see the removed hyprctlProc/gapsOutProc below). niri has no
// live "get current config value" IPC call — there's nothing to poll.
// Rather than hand-parsing `~/.config/niri/config.kdl` to fake the same
// live-mirroring, these are now theme tokens like every other Style value
// (`[style] corner-radius` / `[style] gaps-out` in shell.toml, read through
// the existing styleOverrides/styleNum() mechanism below). This does mean
// the shell's rounding/gap visuals no longer automatically track whatever
// you set in niri's `layout { gaps }` / window-rule `geometry-corner-radius`
// — keep the theme file and your niri config in sync by hand, or pick one
// as the source of truth. This was a deliberate scope cut, not an oversight.
//
...
QtObject {
  ...
  property int cornerRadius: styleNum("corner-radius", 0)
  property int gapsOut: styleNum("gaps-out", 5)
  ...
  // component follows the fontconfig alias `omaniri-font-set` writes.
  ...
  // alias path keeps working when the user runs `omaniri font set`), but
  ...
  // OMANIRI_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("OMANIRI_MENU_FONT")
  ...
  // CHANGED FOR NIRI: was `hyprctlProc.running = true; gapsOutProc.running =
  // true`. cornerRadius/gapsOut are now plain theme-derived properties (see
  // above) that update reactively whenever styleOverrides changes — there
  // is nothing left to poll. Kept as a no-op so the two external callers
  // (shell.qml and Background.qml, both after a theme reload) don't break
  ...
  // hyprctlProc / gapsOutProc: REMOVED FOR NIRI. See the comment on
  // cornerRadius/gapsOut above — niri has no live getoption-style IPC
  // query to poll in the first place.

  // Resolve the fontconfig alias to a concrete family name. `omaniri font
  ...
  // refreshTimer / windowNoGapsToggle: REMOVED FOR NIRI. Both existed only
  // to re-poll hyprctl after Hyprland's async config-reload settled, and to
  // watch the `omaniri toggle window-gaps` Lua flag file that drove it.
  // Neither applies now that gaps/rounding are theme tokens (see above);
  // a niri-side gaps toggle would swap a KDL `layout { gaps }` fragment in
  // niri's own config and does not need to touch this file.
