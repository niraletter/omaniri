...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/omaniri/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/omaniri/shell.toml`
  ...
  // is where `omaniri display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/omaniri/shell.toml"
    watchChanges: true
    printErrors: false
