...
QtObject {
  ...
  property string pluginsDir: home + "/.config/omaniri/plugins"
  ...
  //   - the built-in bar option (`omaniri.bar`) is active when `bar.id` is
  //     missing or set to `omaniri.bar`.
  ...
  //     silently make `omaniri launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "omaniri.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.omaniri) ? manifest.omaniri : null
  ...
  // back, and so a plugin that is both a widget and a menu (omaniri.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "omaniri.workspaces", center: "omaniri.weather", right: "omaniri.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.omaniri)
        ? candidateManifest.omaniri : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "omaniri.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "omaniri.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `omaniri.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("omaniri.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Omaniri plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/omaniri/plugins.
    var script = ""
