...
Item {
  ...
  function launch(desktopId, name) {
    ...
    // Launch straight through gtk-launch — no uwsm-app indirection, so apps run
    // as ordinary detached children of the shell. Keeping gtk-launch as the
    // desktop-entry resolver supports IDs with spaces and entries that uwsm's
    // parser rejected; we no longer depend on that parsing.
  ...
  function remove(desktopId, name) {
    var id = String(desktopId || "")
    if (!id) return
    Util.execDetached(Util.shellQuote(root.omaniriPath + "/bin/omaniri-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
  }
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = root.omaniriPath + "/shell/services/hidden-entries.sh"
  ...
  function closeLaunchFeedback(serial) {
    ...
    if (root.launchOsdOpen) {
      Quickshell.execDetached(["omaniri-shell", "osd", "close"])
  ...
  FileView {
    path: root.omaniriPath + "/default/omaniri/launcher.hides"
    watchChanges: true
    printErrors: false
  ...
  Timer {
    ...
    onTriggered: {
      ...
      Quickshell.execDetached(["omaniri-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: root.launchOsdMessage, duration: 0 })])
    }
  }
