...
`omaniri-shell` is a single long-running [Quickshell](https://quickshell.org/)
instance that hosts the Omaniri desktop. Hyprland autostart launches one shell
...
shell/
  ...
  services/
    ...
    niri/                NiriService singleton — niri IPC bridge (workspaces, windows, outputs)
  plugins/
...
| `bar`        | A full bar option that can replace the built-in `omaniri.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `omaniri.bar`, so users always have a safe path home.
...
clones it straight into `~/.config/omaniri/plugins/<id>/` (named by the
manifest id); updating is a fast-forward pull of that checkout.

```bash
omaniri plugin add https://github.com/acme/omaniri-weather.git
omaniri plugin update acme.weather       # fetches, shows a diff, fast-forwards
omaniri plugin update                    # updates every git-managed plugin
omaniri plugin remove acme.weather
```

> ⚠️ **Plugins run as unsandboxed code inside `omaniri-shell`.** Adding warns
...
```bash
omaniri plugin add https://github.com/acme/omaniri-weather.git --enable --yes
omaniri plugin update --yes
```
...
1. Put it in `~/.config/omaniri/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `omaniri-shell shell rescanPlugins`.
3. `omaniri plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `omaniri bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `omaniri-shell shell rescanPlugins`,
`omaniri-shell shell enablePlugin <id> '{}'`, and `omaniri-shell shell listPlugins`.
The `omaniri plugin` commands wrap those calls. `omaniri bar move` and
`omaniri bar set` edit the persisted widget layout in `shell.json`.
...
automatically; `omaniri-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `omaniri-menu` uses the
shell target to summon the first-party `omaniri.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/omaniri/shell.json`                 |
...
```
quickshell ipc -p $OMANIRI_PATH/shell call shell ping
```

Hyprland autostart launches the shell directly with `quickshell -p
$OMANIRI_PATH/shell`. Use `omaniri-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`omaniri-shell`](../bin/omaniri-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
omaniri-shell shell ping
omaniri-shell shell toggle omaniri.menu '{"menu":"root"}'
omaniri-shell shell listPlugins
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/omaniri/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/omaniri/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/omaniri/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left":   [ { "id": "omaniri.menu" }, { "id": "omaniri.workspaces" } ],
      "center": [ { "id": "omaniri.clock", "format": "HH:mm" } ],
      "right": [
        { "id": "omaniri.audio" }
      ]
    }
...
1. **The active bar option is `bar.id`.** Omit it or set it to `omaniri.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `omaniri.clock`,
   `omaniri.audio`, and `omaniri.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"omaniri.clock", "timezone": ...}`
...
- Phase 1 — `omaniri-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `omaniri-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `omaniri-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `omaniri-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `omaniri-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `omaniri-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `omaniri-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `omaniri-shell phase 8a: unified shell.json with inline plugin settings`
