...
PopupWindow {
  ...
  // "click" — clicking outside the popup dismisses it (see clickCatcher).
  ...
  // Outside-click dismissal. CHANGED FOR NIRI: was HyprlandFocusGrab
  // (hyprland-focus-grab-v1), a Hyprland-only protocol niri doesn't
  // implement, and NiriService has no input-grab action either. Portable
  // fallback: a transparent full-screen layer surface mapped while the popup
  // is open catches outside clicks and closes it. Skipped for hover-mode
  // popups so the cursor can move freely between the trigger and the popup.
  // Assumption: unlike the grab, the catcher also covers the bar, so a
  // second click on the trigger dismisses from here instead of re-entering
  // the widget.
  property PanelWindow clickCatcher: PanelWindow {
    ...
    WlrLayershell.namespace: "omaniri-popup-catcher"
