...
// CHANGED FOR NIRI: `ext-idle-notify-v1` (idleMonitor below, unchanged --
// it's a protocol-level Wayland feature niri implements same as Hyprland)
// still does the actual idle detection. Only the screensaver-window-
// lifecycle tracking below (handleHyprlandEvent, watching Hyprland's
// `openwindow`/`closewindow` raw events for the shell's own screensaver
// surface) needed replacing -- that used Quickshell's built-in Hyprland
// event stream, which niri has no equivalent of in Quickshell. It now
// listens to NiriService's WindowOpenedOrChanged/WindowClosed instead.
...
Item {
  ...
  readonly property string stayAwakeStateDir: home + "/.local/state/omaniri/indicators"
  ...
  readonly property string screensaverClass: "org.omaniri.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("omaniri idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(omaniri-shell lock isLocked 2>/dev/null) == \"true\" ]] || omaniri-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "omaniri-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "omaniri-system-wake")
  ...
  // CHANGED FOR NIRI: was handleHyprlandEvent(event), matching Hyprland's
  // openwindow/closewindow raw-event strings against screensaverClass by
  // parsing comma-separated fields (eventParts/IdleModel). niri's
  // WindowOpenedOrChanged/WindowClosed events carry structured window
  // objects (id, app_id) instead -- no string parsing needed. Windows are
  // tracked by niri's numeric id rather than Hyprland's hex address string;
  // screensaverWindows below is keyed the same way either compositor, so no
  // other function needed to change.
  ...
  function handleNiriWindowClosed(id) {
    var key = String(id)
    if (root.screensaverWindows[key]) root.handleScreensaverWindowClosed(key)
  }
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/omaniri/indicators\" && touch \"$HOME/.local/state/omaniri/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/omaniri/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/omaniri/indicators\"; if [[ -f $HOME/.local/state/omaniri/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
