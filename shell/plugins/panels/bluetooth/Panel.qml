...
Panel {
  id: root
  moduleName: "omaniri.bluetooth"
  ipcTarget: "omaniri.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/omaniri-bluetooth-device;
  ...
  function setDefaultAudioSink(sink) {
    ...
    if (sink.id !== undefined && sink.name) {
      Quickshell.execDetached([
        "omaniri-audio-output-set-default",
        String(sink.id),
        String(sink.name)
      ])
    }
  ...
  function deviceCommand(action, address) {
    return ["omaniri-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via omaniri-bluetooth-device.
  ...
  // the adapter came back on at the next boot. omaniri-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["omaniri-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "omaniri.bluetooth"
