...
// omaniri-network-status when the caller doesn't know it.
Item {
  id: root
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omaniri.speedtest")
    else close()
  }
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = ["omaniri-network-speedtest", nextPhase]
  ...
  Process {
    id: statusProc
    command: ["omaniri-network-status"]
  ...
  SpeedTestOverlay {
    ...
    layerNamespace: "omaniri-network-speedtest"
