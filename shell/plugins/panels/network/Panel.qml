...
Panel {
  id: root
  moduleName: "omaniri.network"
  ipcTarget: "omaniri.network"
  ...
  // `omaniri-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `omaniri-network-band`. `bandCurrent` is the band
  ...
  // The share card is its own panel plugin (omaniri.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("omaniri.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["omaniri-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["omaniri-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (omaniri.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("omaniri.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "omaniri-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "omaniri-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["omaniri-network-status", "--verbose"]
