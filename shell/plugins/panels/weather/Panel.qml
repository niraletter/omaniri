...
Panel {
  id: root
  moduleName: "omaniri.weather"
  ipcTarget: "omaniri.weather"
  manageIpc: false
  ...
  // omaniri-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/omaniri/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["omaniri-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["omaniri-weather-location", "--set", name]
    else
      locationSaveProc.command = ["omaniri-weather-location", "--clear"]
  ...
  // Mirrors omaniri-weather-icon's wttr.in code → nerd-font glyph mapping.
