# Tailscale Omaniri Widget

Native Omaniri bar widget for Tailscale.
...
`omaniri-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `omaniri tailscale receive`
...
This widget ships as first-party plugin `omaniri.tailscale`. Add it with `omaniri plugin enable omaniri.tailscale`, then place it with `omaniri bar move omaniri.tailscale` if desired.
