...
Panel {
  id: root
  moduleName: "omaniri.monitor"
  ipcTarget: "omaniri.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (omaniri-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["omaniri-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("omaniri.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  // CHANGED FOR NIRI: was `hyprctl keyword monitor <name>,disable|preferred,auto,auto`.
  // niri's Output request wants a JSON action, not a keyword string; enable
  // vs. disable maps to On/Off. Handled via NiriService, no subprocess needed.
  function toggleDisplay(name, enabled) {
    if (!name) return
    if (enabled && root.enabledDisplayCount <= 1) return

    NiriService.setOutputEnabled(name, !enabled)
    refreshAfterAction.restart()
  }

  // CHANGED FOR NIRI: was `omaniri-hyprland-monitor-scaling <scale>`, which
  // implicitly targeted whatever Hyprland reported as the *focused* monitor
  // (`hyprctl monitors -j | jq '.[] | select(.focused==true)'`). niri's
  // Output IPC action requires an explicit output name, so this now reads
  // NiriService.currentOutput (the output holding the focused workspace) to
  // reproduce the same "acts on whatever you're looking at" behavior. Note:
  // the old script also supported "up"/"down" step arguments walking a
  // SCALES ladder and logged an audit trail to
  // ~/.local/state/omaniri/monitor-scaling.log -- neither is reproduced
  // here; this panel already only ever called it with an absolute value
  // (see ScalePill.onClicked), so that's the only path ported.
  function setScale(scale) {
    if (!NiriService.currentOutput) return
    NiriService.setOutputScale(NiriService.currentOutput, Number(scale))
    refreshAfterAction.restart()
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["omaniri-display-text-size", String(px)]
  ...
  Process {
    ...
    // `omaniri-brightness-display` races the hardware/driver and can
  ...
  // CHANGED FOR NIRI: was `actionProc` (a hyprctl/bash Process whose
  // onRunningChanged triggered root.refresh() once the compositor command
  // finished). niri actions now fire instantly over the already-open
  // NiriService IPC socket with no process to wait on, so this is a short
  // fixed delay instead of a completion callback -- gives niri a moment to
  // apply the change and push OutputsChanged before re-polling.
  Timer {
