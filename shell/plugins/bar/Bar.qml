...
Item {
  ...
  // The omaniri-shell host injects omaniriPath from OMANIRI_PATH.
  required property string omaniriPath
  ...
  property string omaniriConfigDir: home + "/.config/omaniri"
  ...
  // to "monospace"), so changing the system font (via `omaniri-font-set`)
  ...
  // panel belongs. CHANGED FOR NIRI: was `Hyprland.focusedMonitor`; niri's
  // focused output is tracked by NiriService as the output of the focused
  // workspace. Empty until it reports one, which leaves panel routing on
  // its per-monitor fallback rather than guessing at an output.
  function focusedScreenName() {
    var output = NiriService.currentOutput
    return output ? String(output) : ""
  }

  // Resolve the live bar-widget instance for a plugin id (e.g. "omaniri.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, omaniriConfigDir)
  ...
  FileView {
    path: root.stateHome + "/omaniri/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `omaniri-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/omaniri/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/omaniri/toggles"
    watchChanges: true
    printErrors: false
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "omaniri-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "omaniri-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "omaniri-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
