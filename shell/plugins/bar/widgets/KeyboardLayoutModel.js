...
//
// CHANGED FOR NIRI: the original file existed mostly to solve a Hyprland
// problem that niri doesn't have. Hyprland's `hyprctl devices -j` reports
// keyboards *per physical/virtual device* on the seat (including fake
// "keyboards" like the power button, lid switch, and fcitx5's virtual
// injection device), each independently tracking its own layout index, so
// the widget needed `UNTYPED_KEYBOARDS` filtering + `selectKeyboard`
// "furthest advanced wins" heuristics to figure out which one a human was
// actually typing on.
//
// niri's IPC has no such thing: `KeyboardLayoutsChanged` /
// `KeyboardLayoutSwitched` report ONE global layout list + current index for
// the whole seat (see NiriService.qml). There is nothing to disambiguate.
// The widget no longer calls `eventKeyboardName`, `isTypedKeyboard`, or
// `selectKeyboard`; they are kept at the bottom of this file ONLY because
// test/shell.d/keyboard-layout-test.sh still exercises them and they remain
// part of this module's exported API -- drop them together with that test's
// Hyprland-era sections when the suite is rewritten for niri.
// `layoutBriefs` and `shortLabel` are unchanged (they only ever depended on
// `xkbcli list` output, which is compositor-agnostic and was never actually
// Hyprland-specific despite the old comment referencing "hyprctl reports").

// xkbcli list prints YAML, and every layout and variant block pairs a brief with
// the description the compositor reports as the active keymap:
//
//   - layout: 'us'
//     variant: ''
//     brief: 'en'
...
// ---- Hyprland-only helpers below: unused by the niri widget, kept for the
// node test suite and this module's exported API (see header comment). ----
