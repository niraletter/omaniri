...
// omaniri-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omaniri/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["omaniri-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/omaniri/agents-disabled.json"
  ...
  // Snapshots keep the field names older Omaniri versions wrote, so a fleet
