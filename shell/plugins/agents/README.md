...
`omaniri-agent-usage-update` writes to `~/.local/state/omaniri/agents/usage/`
...
`omaniri plugin disable omaniri.agents`.

## Data

Each agent is one JSON record in `~/.local/state/omaniri/agents/usage/`,
written by `omaniri-agent-usage-update`. That command runs one
`omaniri-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/omaniri/agents/fireworks.json`:
...
- IPC: `omaniri-shell omaniri.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/omaniri/shell.json`. The
top-level keys can be set with
`omaniri bar set omaniri.agents <key> <value>`:
...
```bash
omaniri bar set omaniri.agents refreshIntervalSec 300 --json
omaniri bar set omaniri.agents syncDir '~/Sync/agent-usage'
```
...
```bash
omaniri bar set omaniri.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
