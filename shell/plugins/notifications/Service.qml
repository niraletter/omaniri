// Notification service for the omaniri shell.
...
Item {
  ...
  property string omaniriPath: Quickshell.env("OMANIRI_PATH")
  ...
  readonly property string stateDir: home + "/.local/state/omaniri/"
  ...
  // CHANGED FOR NIRI: was live-mirrored from Hyprland's decoration:rounding
  // via hyprctl polling in Style.qml; niri has no equivalent live query, so
  // Style.cornerRadius is now theme-driven instead. See Commons/Style.qml.
  readonly property int cornerRadius: Style.cornerRadius
  // Toasts are fixed to the top-right corner. They only clear the omaniri bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "omaniri-notifications"
  ...
  // and rare.
  //   - omaniri-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always omaniri or system shell scripts —
  ...
  //   - app_name is "omaniri-action" (Omaniri's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. Omaniri's own toasts carry the
  ...
  // CHANGED FOR NIRI: was a shell-out to bin/omaniri-hyprland-focus-app
  // (hyprctl clients -j + hyprctl dispatch focuswindow). NiriService already
  // keeps a live windows list from the event stream, so this is now an
  // in-process lookup + one IPC call -- no subprocess needed at all.
  // windowByAppId does case-insensitive regex matching against app_id, then
  // falls back to title, same as the old helper's stated behavior.
  function focusApp(entry) {
    if (!entry || !entry.app) return
    const win = NiriService.windowByAppId(String(entry.app))
    if (win) NiriService.focusWindow(win.id)
  }
  ...
  // toasts survive shell restarts (notably the restart `omaniri-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "omaniri-notifications"
