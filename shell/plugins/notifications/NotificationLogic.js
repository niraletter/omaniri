...
function glyphFromHints(hints) {
  return stringHint(hints, "omaniri-glyph")
}

// Shell command to run when the card is clicked, sent by
// omaniri-notification-send --exec. Carrying the action as data means it
...
function execFromHints(hints) {
  return stringHint(hints, "omaniri-exec")
}
...
// ~/.local/state/omaniri/notifications/ so toasts survive shell restarts
// (e.g. the restart `omaniri-update` performs). The file exists exactly as
...
// senders (all Omaniri web apps) delete their scoped /tmp files on close,
