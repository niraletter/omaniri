...
Item {
  ...
  // Injected by omaniri-shell when this plugin is summoned.
  property string omaniriPath: Quickshell.env("OMANIRI_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `omaniri-shell shell summon omaniri.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: omaniriPath + "/default/omaniri/omaniri-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/omaniri/extensions/omaniri-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(omaniri-font-current 2>/dev/null); omaniri-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "omaniri-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); omaniri-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "omaniri-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `omaniri-shell shell summon omaniri.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `omaniri menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/omaniri/extensions/omaniri-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omaniri-menu"
