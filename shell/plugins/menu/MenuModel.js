...
// against `omaniri-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "omaniri-channel-current",
  "omaniri-default-agent",
  "omaniri-default-browser",
  "omaniri-default-editor",
  "omaniri-default-terminal",
  "omaniri-dns"
]
...
function guardHelpers() {
  return 'declare -A __omaniri_pkgs=()\n'
    + 'mapfile -t __omaniri_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __omaniri_pkg in "${__omaniri_pkg_names[@]}"; do __omaniri_pkgs[$__omaniri_pkg]=1; done\n'
    + '__omaniri_pkg_has() { [[ -n ${__omaniri_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'omaniri-pkg-present() { local p; for p in "$@"; do __omaniri_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'omaniri-pkg-missing() { local p; for p in "$@"; do __omaniri_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'omaniri-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'omaniri-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__omaniri_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
