...
# ISO chroot — by first-run, the Niri session is up and
...
systemctl --user enable --now \
  bt-agent.service \
  omaniri-recover-internal-monitor.service \
  omaniri-sleep-lock.service \
  omaniri-migrate-notify.service \
  omaniri-fcitx5.service \
  omaniri-crash-watch.service
