...
if omaniri-hw-match "XPS" && omaniri-hw-intel-ptl; then
  echo "Detected Dell XPS Panther Lake, installing PTL kernel..."

  omaniri-pkg-add linux-ptl linux-ptl-headers
  ...
  # Named to sort after omaniri-defaults.conf: drop-ins are read in order and
