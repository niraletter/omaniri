...
# Meteor Lake (170/172), Lunar Lake (189)

if omaniri-hw-intel && omaniri-battery-present; then
  cpu_model=$(grep -m1 "^model\s*:" /proc/cpuinfo 2>/dev/null | cut -d: -f2 | tr -d ' ')
  if [[ "$cpu_model" =~ ^(151|154|170|172|183|186|189|191)$ ]]; then
    omaniri-pkg-add intel-lpmd
